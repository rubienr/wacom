...
class ConfigName(object):
    def __init__(self, base_path: str, file_name: str):
        self.config_name = file_name.removesuffix(".py")
...
class ConfigLoader(object):
    ...
    def config_names(self) -> List[ConfigName]:
        return [ConfigName(self.path_to_config_folder, file_name) for file_name in ConfigLoader._py_files(self.config_path) if "base_config.py" not in file_name and "__init__.py" not in file_name]
    ...
    def _py_files(config_path: str) -> List[str]:
        return [f for f in os.listdir(config_path) if os.path.isfile(os.path.join(config_path, f)) and f.endswith(".py")]
